    ...
    Insert() {
        ...
        var y= this;
         for(var i=35; i>-1; i=i-7){
            if(document.getElementsByClassName("dot")[parseInt(x.id) + i].classList.contains('red') || document.getElementsByClassName("dot")[parseInt(x.id) + i].classList.contains('blue'))
                ;
            else {
                document.getElementsByClassName("dot")[parseInt(x.id) + i].classList.add(y.color);
                if(y.color=="red")
                    y.color="blue";
                else
                    y.color="red";
         ...
         if((parseInt(x.id) + i) > -1)
         this.Verify(parseInt(x.id) + i);
      },
      Verify(index){
            var count=1;
            if(this.color=="red")
            ...
            for(var j=1; j<iteration+1; j++) {
                if(document.getElementsByClassName("dot")[index + j].classList.contains(testcolor))
                count++;
                else
                    break;
            }
            for(var i=1; i<indexplace+1; i++) {
                if(document.getElementsByClassName("dot")[index - i].classList.contains(testcolor))
                count++;
                else 
                    break;
            }
            if(count < 4) 
                this.VerifyVertical(index, testcolor);
                else {
                this.Win(testcolor);
                }
      },
      VerifyVertical(index, testcolor) {
            ...
            for(var i=0 ; i<iteration ; i++) {
               ...
               if(document.getElementsByClassName("dot")[newindex].classList.contains(testcolor)) 
                    count++;
                else 
                    break;
            }
                if(count < 4)
                    this.VerifyRight(index, testcolor);
                    else
                    this.Win(testcolor);
      },
      VerifyRight(index, testcolor){
          ...
          //} else {
              for(var j = 0 ; j< 3 ; j++) {
                    ...
                    if(newindex2 >-1) {
                    if(document.getElementsByClassName("dot")[newindex2].classList.contains(testcolor)) 
                        count++;
                    else 
                        break;
              }}
          //}
          if(count < 4)
            this.VerifyLeft(index,testcolor);
            else
            this.Win(testcolor);
      },
